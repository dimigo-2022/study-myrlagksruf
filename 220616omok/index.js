...
function myFunction(e) {
    ...
    if (e.range.getLastColumn() !== col || e.range.getLastRow() !== row || col > 26 || row > 26)
        return;
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();
    const userRange = sheet.getRange(2, 27);
    let user = userRange.getValue().toUpperCase();
    if (!user)
        user = 'O';
    ...
    const obj = { 'O': 'X', 'X': 'O' };
    const range = sheet.getRange(Math.max(1, row - 4), Math.max(1, col - 4), Math.min(9, row + 4), Math.min(9, col + 4)).getValues();
    ...
    for (let [dx, dy] of way) {
        ...
        big: for (; typeof range[curSero]?.[curGaro] !== 'undefined'; [curGaro, curSero] = [curGaro - dx, curSero - dy]) {
            ...
            if (compare.toUpperCase() !== 'O' && compare.toUpperCase() !== 'X')
                continue big;
            for (let i = 1; i < 5; i++) {
                if (compare !== String(range[curSero + dy * i]?.[curGaro + dx * i]))
                    continue big;
            }
