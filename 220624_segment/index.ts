import { IndexedTree, DrawInfo } from './indexedtree.js';
...
function drawing(draw:DrawInfo){
    ctx.save();
...
function changePos(draw:DrawInfo){
    ...
    const pos = [sx, sy, ex, ey] as [number, number, number, number];
    return {
...
canvas.addEventListener('mousemove', e => {
    if(e.buttons % 2 === 1){
        if(isDrawStart()){
            ...
            draw.pos = [...(draw.pos.slice(0, 2) as [number, number]), e.offsetX, e.offsetY];
        }
    } else {
